...
#define ITERATE_ONE_HOP_NBRS_BEGIN(node, nbrnode) { \
	node_t *_current_node = NULL; \
	ITERATE_NBRS_BEGIN(node, _current_node) { \
		ITERATE_NBRS_BEGIN(_current_node, nbrnode) { \
			if (nbrnode == node) \
				continue;
